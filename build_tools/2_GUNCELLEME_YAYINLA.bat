...
where gh >nul 2>&1
if not errorlevel 1 set "GH_CMD=gh"
if errorlevel 1 if exist "C:\Program Files\GitHub CLI\gh.exe" set "GH_CMD=C:\Program Files\GitHub CLI\gh.exe"
if errorlevel 1 if exist "C:\Program Files (x86)\GitHub CLI\gh.exe" set "GH_CMD=C:\Program Files (x86)\GitHub CLI\gh.exe"
...
"%GH_CMD%" release create "v%APP_VERSION%" ^
    "%INSTALLER%" ^
    --repo "%GITHUB_REPO%" ^
...
"%GH_CMD%" release view "v%APP_VERSION%" --web --repo "%GITHUB_REPO%" 2>nul
